...
const ProductList = ({ colClass, layoutList, openSidebar, noSidebar }) => {
  ...
  useEffect(() => {
    ...
    setUrl(pathname);
    router.push(
      // `${pathname}?type=all`, undefined, { shallow: true }
       `${pathname}?art&brand=${selectedBrands}&color=${selectedColor}&size=${selectedSize}&minPrice=${selectedPrice.min}&maxPrice=${selectedPrice.max}`, undefined, { shallow: true }
    );
